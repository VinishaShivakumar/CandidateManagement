...
//        config.addMapping("/**").allowedOrigins("http://localhost:4200")
...
//        String token = tokenStore.generateToken( authentication );
  Map<String,String> token =tokenStore.generateToken(authentication);
//       Map<String,String> m=new HashMap<String,String>();
//       m.put("accessToken", token);
//       m.put("refreshToken", refresh);
        response.getWriter().write(
                mapper.writeValueAsString(token)//Collections.singletonMap( "accessToken", token) )
        );
    }
